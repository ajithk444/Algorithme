...
namespace DynamiqueProgramming
{
    ...
    {
        public static void GetLSS(string str1)
        {
            ...
            HashSet<int> processedIndex = new HashSet<int>();

            for (int i = 1; i < xLength; i++)
            {
                for (int j = 1; j < xLength; j++)
                {
                    if(i!=j && !processedIndex.Contains(i) && !processedIndex.Contains(j) && xChars[i-1] == yChars[j - 1])
                    {
